...
export function termsSections(business: BusinessSettings): LegalSection[] {
  ...
  return [
    ...
    },
    {
      ...
      paragraphs: [
        deposit
          ? `The deposit is paid by credit card at the time of booking, or by Interac e-transfer using the details on your invoice. The remaining ${100 - business.depositPercent}% is charged automatically to the same card ${business.balanceDaysBefore} calendar days before the session; if no card is on file, it is due by e-transfer or card by that date.`
          : 'Payment is made by credit card at the time of booking, or by Interac e-transfer using the details on your invoice.',
